/*
    2019 ... Author: John Tsiombikas <nuclear@member.fsf.org>
    ...
    add compatibility with winpthread.h and visual studio 19
    
    Main project site:     https://github.com/ClaudioDaffra/cx
*/
...
#ifndef C11THREADS_TIMEDLOCK_POLL_INTERVAL
...
static inline int mtx_timedlock(mtx_t *mtx, struct timespec *ts)
{
	int res;
...
static inline int cnd_timedwait(cnd_t *cond, mtx_t *mtx, struct timespec *ts)
{
	int res;

	if(
        (   res = 
            pthread_cond_timedwait(cond, mtx, ts)) != 0
        ) 
    {
